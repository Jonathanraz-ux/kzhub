...
import { Route, Maximize2, X, ZoomIn, ZoomOut, RotateCcw, MapPin } from "lucide-react";
import { useLanguage } from "@/lib/i18n/LanguageContext";

const SITE_LOCATION_IMAGE_SRC = "/images/mining-site-location.jpg";

const MiningSiteLocation: React.FC = () => {
  const { t } = useLanguage();
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8 lg:px-12 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gold-500/10 border border-gold-500/30 text-gold-400 text-xs font-medium tracking-wide mb-4">
            ...
            <span>{t.siteLocationSurtitle}</span>
          </div>
        ...
        >
          <div
            ...
            className="group relative cursor-pointer rounded-2xl overflow-hidden border border-white/15 bg-[#fcfbf9] p-4 sm:p-6 md:p-8 shadow-2xl shadow-black/50 transition-all duration-300 hover:border-gold-500/40 hover:shadow-gold-500/10"
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
            ...
            aria-label={t.siteLocationViewFull}
          >
            {/* Overlay hint button */}
            <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-20 opacity-90 group-hover:opacity-100 transition-opacity">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/75 hover:bg-black/90 text-gold-300 text-xs font-medium backdrop-blur-md border border-white/20 shadow-md transition-colors">
                <Maximize2 size={13} />
                <span>{t.siteLocationViewFull}</span>
              </span>
            </div>
            ...
            <div className="relative w-full flex items-center justify-center bg-white rounded-xl shadow-inner overflow-hidden border border-neutral-200/80 p-2 sm:p-4">
              <img
                src={SITE_LOCATION_IMAGE_SRC}
                alt={t.siteLocationAlt}
                loading="lazy"
                className="w-full h-auto max-h-[720px] object-contain rounded transition-transform duration-500 group-hover:scale-[1.012]"
              />
            </div>
          </div>

          {/* Official Location Caption */}
          <div className="flex items-center justify-center gap-2 mt-4 text-center text-xs md:text-sm text-cream/55 tracking-wide">
            <MapPin size={14} className="shrink-0 text-gold-400/80" />
            <span>{t.siteLocationCaption}</span>
          </div>
        </motion.div>
      </div>
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-black/60">
                <div className="flex items-center gap-2 text-cream text-xs md:text-sm font-medium">
                  <Route size={15} className="text-gold-400 shrink-0" />
                  <span className="truncate">{t.siteLocationTitle}</span>
                </div>
...
export default MiningSiteLocation;
